...
void DrawingCanvas::analyzeImageWindows(){
    // Capture the current canvas as an image
    QPixmap pixmap = this->grab();
    QImage image = pixmap.toImage();
    
    cout << "Starting window analysis..." << endl;
    cout << "Image size: " << image.width() << "x" << image.height() << endl;
    
    // Perform window analysis
    m_imageAnalyzer->analyzeWindows(image);
    
    // Find the ideal window size
    int idealSize = m_imageAnalyzer->findIdealWindowSize();
    cout << "Ideal window size: " << idealSize << "x" << idealSize << endl;
    
    // Generate the report
    m_imageAnalyzer->generateReportMd("report.md");
    cout << "Analysis complete! Report generated as 'report.md'" << endl;
    
    // Print results to console
    QMap<int, int> results = m_imageAnalyzer->getAllResults();
    cout << "\nWindow Analysis Results:" << endl;
    cout << "========================" << endl;
    for (auto it = results.begin(); it != results.end(); ++it) {
        cout << "Window size " << it.key() << "x" << it.key() 
             << ": " << it.value() << " non-empty windows" << endl;
    }
    cout << "========================" << endl;
}
